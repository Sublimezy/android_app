package com.xueyiche.zjyk.xueyiche.practicecar.activity.nobook.view;
...
import com.xueyiche.zjyk.xueyiche.homepage.view.WheelPicker;
import com.xueyiche.zjyk.xueyiche.homepage.view.WheelView;
 ...
 * 两级、三级联动选择器。
 */
public class WZLinkagePicker extends WheelPicker {
    ...
    protected ArrayList<ArrayList<String>> thirdList = new ArrayList<ArrayList<String>>();
    ...
    public WZLinkagePicker(Activity activity) {
        super(activity);
    }
     ...
     * 二级联动选择器构造函数
     */
    public WZLinkagePicker(Activity activity, ArrayList<String> firstList,
    ...
    public WZLinkagePicker(Activity activity, ArrayList<String> firstList,
                           ArrayList<ArrayList<String>> secondList,
                           ArrayList<ArrayList<String>> thirdList) {
        super(activity);
    ...
    public void setSelectedItem(String firstText, String secondText, String thirdText) {
        ...
        ArrayList<String> thirdTexts = thirdList.get(selectedFirstIndex);
    ...
    protected View makeCenterView() {
        ...
        firstView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
            ...
            public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                ...
                secondView.setItems(secondData, 0);
                if (thirdList.size() == 0) {
                    return;//仅仅二级联动
                }
        ...
        secondView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
            ...
            public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                ...
                ArrayList<String> secondData = secondList.get(selectedFirstIndex);
                if (secondData.size() < selectedThirdIndex) {
                    //上一次的第三级选择的项的索引超出了第三级的数据数
                    selectedThirdIndex = 0;
                }
                //根据第2级数据获取第3级数据。若不是用户手动滚动，说明联动需要指定默认项
                String aa = secondData.get(selectedSecondIndex);

                String substring = aa.substring(0, 2);
                BigDecimal a = new BigDecimal(substring);
                ArrayList<String> thirdData = new ArrayList<String>();
                thirdData.add(DateUtils.fillZero(a.intValue() +3));
//                for (int i = a.intValue() + 3 > 21 ? 21 : a.intValue() + 3; i < 22; i++) {
//                    thirdData.add(DateUtils.fillZero(i));
//                }
//                isUserScroll
                thirdView.setItems(thirdData, 0);
            }
        });
        if (thirdList.size() == 0) {
            return layout;
        }
        thirdView.setItems(thirdList.get(selectedSecondIndex), selectedThirdIndex);
