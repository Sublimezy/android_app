 ...
 * 练车管理   日期    教练端
 */
public class TrainingDateManageActivity extends NewBaseActivity implements CalendarView.OnCalendarInterceptListener, CalendarView.OnCalendarSelectListener, CalendarView.OnMonthChangeListener {
    ...
    protected int initContentView() {
        return R.layout.activity_training_date_manage;
    }

    @Override
    protected void initView() {
        ButterKnife.bind(this);
        ImmersionBar.with(this).titleBar(R.id.title_bar_rl).statusBarDarkFont(true).init();
    }
...
//                        showToastShort("切换科目3");
    ...
    private void getDataFromNet() {
        ...
        MyHttpUtils.postHttpMessage(AppUrl.selectcoachdays, params, TrainingDateManageBean.class, new RequestCallBack<TrainingDateManageBean>() {
            @Override
    ...
    @OnClick({R.id.title_bar_back, R.id.title_bar_right_text})
...
//        instance.add(java.util.Calendar.DATE, 10);
//        String days_after = sdf2.format(instance1.getTime());
//        System.out.println(days_after);

//        String[] split = days_after.split("-");

//        mCalendarView.setRange(year, month, curDay,
...
//        return false;
        ...
        return TextUtils.isEmpty(scheme) || "".equals(scheme);
    }
...
//点击日期的时候  联网
        ...
        if ("已设置".equals(calendar.getScheme())) {
            //直接进  不请求接口
            //需要传selected_date_id

            Intent intent = new Intent(TrainingDateManageActivity.this, TrainingTimeCoachActivity.class);
            intent.putExtra("driving_type", driving_type);
            intent.putExtra("coach_user_id", coach_id);
            intent.putExtra("coach_data", selectedDate);
            ...
            params.put("coach_data", selectedDate);
            MyHttpUtils.postHttpMessage(AppUrl.addcoachdate, params, TrainingDataCoachSelectBean.class, new RequestCallBack<TrainingDataCoachSelectBean>() {
                @Override
                public void requestSuccess(TrainingDataCoachSelectBean json) {
                    if (json.getCode() == 200) {
