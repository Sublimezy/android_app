 ...
 */
public class LinkagePickerTwo extends WheelPicker {
    ...
    public LinkagePickerTwo(Activity activity) {
        super(activity);
    }
     ...
     * 二级联动选择器构造函数
     */
    public LinkagePickerTwo(Activity activity, ArrayList<String> firstList,
    ...
    protected View makeCenterView() {
        ...
        secondView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
            ...
            public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                ...
                ArrayList<String> secondData = thirdList.get(selectedSecondIndex);
                if (secondData.size() < selectedThirdIndex) {
                    //上一次的第三级选择的项的索引超出了第三级的数据数
                    selectedThirdIndex = 0;
                }
                selectedThirdIndex = 0;
                thirdView.setItems(secondData, 0);
