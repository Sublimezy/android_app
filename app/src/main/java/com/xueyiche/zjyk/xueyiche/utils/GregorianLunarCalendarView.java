...
public class GregorianLunarCalendarView extends LinearLayout implements NumberPickerView.OnValueChangeListener{
    ...
    private static final int YEAR_STOP = 2050;
    ...
    private Calendar adjustCalendarByLimit(Calendar c, int yearStart, int yearStop, boolean isGregorian){
        ...
        if(isGregorian){
            ...
            if(yearGrego > yearStop){
                ...
                int daySway = ChinaDateUtil.getSumOfDayInMonthForGregorianByMonth(yearStop, MONTH_STOP_GREGORIAN);
            ...
            if(Math.abs(yearGrego - yearStart) < Math.abs(yearGrego - yearStop)){
                ...
                int daySway = ChinaDateUtil.getSumOfDayInMonthForLunarByMonthLunar(yearStop, MONTH_STOP_LUNAR_NORMAL);
    ...
    public void setThemeColor(int themeColor){
        ...
        mYearPickerView.setHintTextColor(0XFFBBBBBB);
        mYearPickerView.setDividerColor(0xFFf2f3f5);
        mMonthPickerView.setSelectedTextColor(themeColor);
        mMonthPickerView.setHintTextColor(0XFFBBBBBB);
        mMonthPickerView.setDividerColor(0xFFf2f3f5);
        mDayPickerView.setSelectedTextColor(themeColor);
        mDayPickerView.setHintTextColor(0XFFBBBBBB);
        mDayPickerView.setDividerColor(0xFFf2f3f5);
    }
    ...
    private void setDisplayData(boolean isGregorian){
        ...
        if(isGregorian){
            ...
            if(mDisplayYearsLunar == null){
                ...
                for(int i = 0; i < YEAR_SPAN; i++){
                    mDisplayYearsLunar[i] = ChinaDateUtil.getLunarNameOfYear(i + YEAR_START);
                }
            }
            if(mDisplayMonthsLunar == null){
                mDisplayMonthsLunar = new String[MONTH_SPAN_GREGORIAN];
                for(int i = 0; i < MONTH_SPAN_GREGORIAN; i++){
                    mDisplayMonthsLunar[i] = ChinaDateUtil.getLunarNameOfMonth(i + 1);
                }
            }
            if(mDisplayDaysLunar == null){
                mDisplayDaysLunar = new String[DAY_SPAN_LUNAR];
                for(int i = 0; i < DAY_SPAN_LUNAR; i++){
                    mDisplayDaysLunar[i] = ChinaDateUtil.getLunarNameOfDay(i + 1);
                }
            }
        }
    }
    ...
    private void initValuesForM(ChineseCalendar cc, boolean isGregorian, boolean anim){
        ...
        if(isGregorian){
            ...
            int monthLeap = ChinaDateUtil.getMonthLeapByYear(cc.get(ChineseCalendar.CHINESE_YEAR));
            if(monthLeap == 0){
                ...
                monthSway = ChinaDateUtil.convertMonthLunarToMonthSway(cc.get(ChineseCalendar.CHINESE_MONTH), monthLeap);
                newDisplayedVales = ChinaDateUtil.getLunarMonthsNamesWithLeap(monthLeap);
            }
        }
    ...
    private void initValuesForD(ChineseCalendar cc, boolean isGregorian, boolean anim){
        if(isGregorian){
            ...
            int dayStop = ChinaDateUtil.getSumOfDayInMonthForGregorianByMonth(cc.get(Calendar.YEAR), cc.get(Calendar.MONTH) + 1);
            ...
            int dayStop = ChinaDateUtil.getSumOfDayInMonthForLunarByMonthLunar(cc.get(ChineseCalendar.CHINESE_YEAR), cc.get(ChineseCalendar.CHINESE_MONTH));
    ...
    private void passiveUpdateMonthAndDay(int oldYearFix, int newYearFix, boolean isGregorian){
        ...
        if(isGregorian){
            ...
            int oldDayStop = ChinaDateUtil.getSumOfDayInMonth(oldYearFix, oldMonthSway, true);
            int newDayStop = ChinaDateUtil.getSumOfDayInMonth(newYearFix, newMonthSway, true);
            ...
            int newYearFixMonthLeap = ChinaDateUtil.getMonthLeapByYear(newYearFix);//1.计算当前year是否有闰月
            int oldYearFixMonthLeap = ChinaDateUtil.getMonthLeapByYear(oldYearFix);//2.计算之前year是否有闰月

            if(newYearFixMonthLeap == oldYearFixMonthLeap){
                //only update day picker
                newMonthSway = oldMonthSway;

                int oldMonthLunar = ChinaDateUtil.convertMonthSwayToMonthLunar(oldMonthSway, oldYearFixMonthLeap);
                int newMonthLunar = ChinaDateUtil.convertMonthSwayToMonthLunar(newMonthSway, newYearFixMonthLeap);
                int oldDayStop = ChinaDateUtil.getSumOfDayInMonthForLunarByMonthLunar(oldYearFix, oldMonthLunar);
                int newDayStop = ChinaDateUtil.getSumOfDayInMonthForLunarByMonthLunar(newYearFix, newMonthLunar);
                ...
                //月视图需要更新
                mCurrDisplayMonthsLunar = ChinaDateUtil.getLunarMonthsNamesWithLeap(newYearFixMonthLeap);

                //优化方案
                int oldMonthLunar = ChinaDateUtil.convertMonthSwayToMonthLunar(oldMonthSway, oldYearFixMonthLeap);
                int oldMonthLunarAbs = Math.abs(oldMonthLunar);
                newMonthSway = ChinaDateUtil.convertMonthLunarToMonthSway(oldMonthLunarAbs, newYearFixMonthLeap);
                ...
                //日视图需要更新
                int oldDayStop = ChinaDateUtil.getSumOfDayInMonth(oldYearFix, oldMonthSway, false);
                int newDayStop = ChinaDateUtil.getSumOfDayInMonth(newYearFix, newMonthSway, false);
                if(oldDayStop == newDayStop){
                    if(mOnDateChangedListener != null){
                        mOnDateChangedListener.onDateChanged(getCalendarData(newYearFix, newMonthSway, oldDaySway, isGregorian));
                    }
    ...
    private void passiveUpdateDay(int oldYear, int newYear, int oldMonth, int newMonth, boolean isGregorian){
        ...
        int oldDayStop = ChinaDateUtil.getSumOfDayInMonth(oldYear, oldMonth, isGregorian);
        int newDayStop = ChinaDateUtil.getSumOfDayInMonth(newYear, newMonth, isGregorian);
    ...
    public static class CalendarData{
        ...
        private void initChineseCalendar(){
            if(isGregorian){
                ...
                int y = pickedYear;
                int m = ChinaDateUtil.convertMonthSwayToMonthLunarByYear(pickedMonthSway, pickedYear);
                int d = pickedDay;
