...
import com.xueyiche.zjyk.xueyiche.homepage.view.LinkagePickerTwo;
...
public class NowPracticeActivity extends BaseActivity implements View.OnClickListener {

    //新版
    //开始时间
    private RelativeLayout rl_time_practice;
    //结束时间
    ...
    //练车时长
    private ArrayList<String> timePriceList;
    ...
    private TextView tv_login_back, tv_duration;
    ...
    protected int initContentView() {
        return R.layout.now_practice_activity;
    }
...
//        ll_choose_type = (LinearLayout) view.findViewById(R.id.title).findViewById(R.id.ll_choose_type);
        ...
        rl_time_practice = (RelativeLayout) view.findViewById(R.id.rl_time_practice);
    ...
    protected void initListener() {
        rl_time_practice.setOnClickListener(this);
...
//                startActivityForResult(intent3, 3);
    ...
    private void choseTime() {
        final OptionPicker picker = new OptionPicker(this, timePriceList);
        picker.setOffset(1);
        ...
        picker.setOnOptionPickListener(new OptionPicker.OnOptionPickListener() {
            ...
            public void onOptionPicked(int position, String option) {
                ...
                practice_hour = timePriceList.get(position);
                tv_duration.setText(option + "小时");
            }
    ...
    private void fabu() {
        ...
        String totlaTime = tv_duration.getText().toString().trim();
        String true_phone = tv_urgency_phone.getText().toString().trim();
        if (TextUtils.isEmpty(car_level)) {
            Toast.makeText(this, "请选择车辆价格", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (TextUtils.isEmpty(totlaTime)) {
            Toast.makeText(this, "请选择练车时长", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        String end_time = formatter.format(endtime);
        OkHttpUtils.post()
                .url(AppUrl.Order_Indent_Post)
                .addParams("device_id", LoginUtils.getId(this))
                .addParams("user_id", user_id)
                .addParams("car_level", car_level)
                .addParams("start_time", dangqian_time)
                .addParams("end_time", end_time)
                .addParams("driver_id", "")
                .addParams("order_sort", "1")
                .addParams("practice_hours", practice_hour)
    ...
    public void carStyle() {
        if (firstList.size()!=0&&secondList.size()!=0&&thirdList.size()!=0) {
            final LinkagePickerTwo picker = new LinkagePickerTwo(this, firstList, secondList, thirdList);
            picker.setTitleText("请选择车辆级别");
            picker.setTitleTextSize(20);
            picker.setTitleTextColor(getResources().getColor(R.color._3232));
            picker.setOnLinkageListener(new LinkagePickerTwo.OnLinkageListener() {
                @Override
                public void onPicked(String first, String second, String third) {
                    tv_carstyle_jibie.setText(first + "-" + second + "-" + third);
                    if (firstList.contains(first)) {
                        car_level = "" + (firstList.indexOf(first));
                    }
                    picker.dismiss();
                }
            });
            ...
            Toast.makeText(NowPracticeActivity.this, "暂无车型信息", Toast.LENGTH_SHORT).show();
        }
