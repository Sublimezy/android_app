...
public final class ChineseCalendar extends GregorianCalendar {
    ...
    public static final int startTime  = 1980;
    public static final int endTime = 2050;
    ...
    private void computeGregorianFields() {
        ...
        if (dateint < 19001111) { // too small
            set(startTime, Calendar.JANUARY, 1);
            super.complete();
        } else if (dateint > 21001201) { // too large
            set(endTime, Calendar.DECEMBER, 31);
            super.complete();
        } else {
            if (Math.abs(m) > 12) {
                m = 12;
    ...
    private void computeChineseFields() {
        ...
        if (gregorianYear < startTime || gregorianYear > endTime) {
            return;
        }
